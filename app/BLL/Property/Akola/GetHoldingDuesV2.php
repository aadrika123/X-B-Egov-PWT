    ...
    public function getDues($req)
    {
        ...
        $demandList = $mPropDemand->getDueDemandByPropIdV2($req->propId)->map(function ($val) {
            ...
            if ($this->_isMobileTower && $this->_isSingleManArmedForce) {
                ...
                $val->due_exempted_general_tax = $this->_isMobileTower ? ($val->due_general_tax) : 0;
            }
            return $val;
        });
        ...
        foreach ($demandList as $list) {
            $calculate2PercPenalty->calculatePenalty($list, $propBasicDtls->prop_type_mstr_id, $this->_isSingleManArmedForce, $this->_isMobileTower);
        }
        ...
        $basicDtls["applicant_name"]     = $basicDtls->implode("applicant_name", ",") ??$firstOwner->guardian_name_marathi ?? $firstOwner->guardian_name;
