    ...
    public function getByItsDetailsV2($req, $key, $refNo,)
    {
        return self::select(
          ...
          END AS approval_status"),
            DB::raw("CASE 
            WHEN active_trade_temp_licences.current_role_id = 6 THEN 'AT LIPIK'
            WHEN active_trade_temp_licences.current_role_id = 13 THEN 'AT SECTION INCHARGE'
            WHEN active_trade_temp_licences.current_role_id = 10 THEN 'AT TAX  SUPRERINTENDENT'
            ELSE 'Unknown Role'
        END AS application_at"),
            "active_trade_temp_licences.payment_status"
        )
            ->join('wf_roles', 'wf_roles.id', '=', 'active_trade_temp_licences.current_role_id')
            ->leftjoin('ulb_ward_masters', 'ulb_ward_masters.id', '=', 'active_trade_temp_licences.ward_mstr_id')
