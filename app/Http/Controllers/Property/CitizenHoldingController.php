...
class CitizenHoldingController extends Controller
{
    ...
    public function ICICPaymentRequestProcessFee(Request $request)
    {
        ...
        try {
            ...
            $response = $this->_IciciPaymentController->getReferalUrl($request);

            if (!$response->original["status"]) {
                throw new Exception("Payment not initiated");
            }

            $plainUrl = $response->original["message"]["plainUrl"];
            $response = $response->original["data"];

            $request->merge([
                "encryptUrl" => $response["encryptUrl"],
                "reqRefNo" => $response["req_ref_no"]
            ]);

            $response["propId"] = $request->propId;
            $response["plainUrl"] = $plainUrl;
            $response["paidAmount"] = $request->paidAmount;
            $response["amount"] = $request->amount;

            DB::beginTransaction();
            $response["requestId"] = $this->_PropIciciPaymentsRequest->store1($request);
            DB::commit();

            $response = collect($response)->only(
                [
                    "encryptUrl",
                    "propId",
                    "paidAmount",
            ...
            return responseMsgs(true, "Request initiated", $response, "phc1.1", "1.0", "", "POST", $request->deviceId ?? "");
        } catch (Exception $e) {
            DB::rollBack();
            return responseMsgs(false, $e->getMessage(), $request->all(), "phc1.1", "1.0", "", "POST", $request->deviceId ?? "");
        }
